...
# 以下是需要的小函数
    ...
    def generate_record(self): # 从抓取的原始素材产生记录
        ...
        while(m < len(self.pmid)):  # 有多少重复多少
            ...
            journal_end = str(self.journal[m-1]).find(journal_end_with) # 期刊结尾位置
            journal = str(self.journal[m-1])[26:journal_end].replace('<b>', '').replace('</b>', '')  # 期刊
            pmid = str(self.pmid[m-1])[4:-5]  # pmid
            title_start = str(self.title[m-1]).find(title_start_with) + 22
            title = str(self.title[m-1])[title_start:-8].replace('<b>', '').replace('</b>', '')  # 论文名
            issue = re.search("[1-2][09][0-9]{2}", str(self.issue[m-1])).group(0)  # 刊号
            ...
            if not(self.pmid_check(pmid)): # 如果之前没有这篇文章
                ...
                if detail: # 如果能够返回正确的abstract，记录；否则留给下一次抓取（不记录，视作新论文）
                    record = self.time_now(),self.key_words,pmid,title,journal,author,issue,detail[0],detail[1],detail[2],detail[3] 
...
# 实际爬的部分开始
    def crawl_direct(self): # 用于直接爬sum-page，只能爬第一页
        ...
        self.crawl_start_time = self.time_now()
        ...
        while(tries > 0):
            try:
                ...
                max_sum_page_number_raw = soup.findAll(name = "input", attrs={"id":"pageno"})
    ...
    def crawl_phantom(self): # 用于使用phantomjs爬取sum-page，可以爬无限页，但是速度慢
        ...
        self.crawl_end_time = self.time_now()
        if self.run_type:
            self.project_sum()
